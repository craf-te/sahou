...
fn py_files_all(
    ...
    let overload_sigs: Vec<String> = nodes
        ...
        .collect();
    // Module-level overloads: @overload sits at column 0 (unlike py_method, which indents for class methods).
    // mypy errors on a lone @overload, so a single node gets a plain def.
    let overloads = if overload_sigs.len() == 1 {
        overload_sigs[0].clone()
    } else {
        overload_sigs
            .iter()
            .map(|s| format!("@overload\n{s}"))
